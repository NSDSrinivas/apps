...
4. **Wire it into the hub.** Add or update an `<li class="app-card">` entry in the root `index.html`'s `<ul class="app-grid">`, matching the existing markup (icon, name, one-line tagline, "View app" + Privacy Policy links). Remove the "New" badge once an app isn't new anymore, or add it for a freshly added one.
